#!/bin/bash
#
# Start script for payment-processed-consumer
...
if [[ -z "${MESOS_SLAVE_PID}" ]]; then
    ...
    source ~/.chs_env/payment-processed-consumer/env
...
exec java ${JAVA_MEM_ARGS} -jar -Dserver.port="${PORT}" "${APP_DIR}/payment-processed-consumer.jar"